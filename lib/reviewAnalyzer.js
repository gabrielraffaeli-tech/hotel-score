...
function contarMencionesEnReseña(texto, keywords, rating = null) {
  ...
  // Si la reseña tiene calificación alta (4-5★), reducir peso de negativos
  // Si tiene calificación baja (1-2★), reducir peso de positivos
  if (rating) {
    if (rating >= 4) negativas *= 0.4
    else if (rating <= 2) positivas *= 0.4
  }
...
export function analizarReseñas(reseñas) {
  ...
  for (const [key, dim] of Object.entries(DIMENSIONS)) {
    // Analizar reseña por reseña con su rating para mejor precisión
